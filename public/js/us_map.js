...
(function () {
  ...
  /* Read topojson data */
  d3.queue()
    .defer(d3.json, "data/mapdata/us.topojson")
    .await(ready);

  /* Create projection */
  var projection = d3.geoAlbersUsa().translate([430, 250]).scale(850);
    ...
    d3.selectAll("path").each((d) => {
      firebase.firestore().collection("users").where("state", "==", d.properties.name).get().then((result) => {
        if (result.docs.length > 0) {
          populated.push(d.properties.cartodb_id);
        }
      });
    });
    setTimeout(() => {
    ...
    userRef.where("country", "==", country)
          ...
          .then(docs => {
            ...
            if (nameListApp.userList.length > 0) {
              nameListApp.message = "We found " + nameListApp.userList.length + " people in " + state
                                    + ". Click on their names to find more about him/her.";
            } else {
              nameListApp.message = "Oops! There seems no 1522er in " + state + " now.";
            }
  ...
  userDocRef.get()
            .then(doc => {
              ...
              if (doc.data().state !== "" && doc.data().state !== "None") {
                displayUserApp.location = doc.data().city + ", " + doc.data().state + ", US";
              } else {
